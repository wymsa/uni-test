...
import { GetEventsFilter, GetRevenueFilter, ReporterEvents, TiktokEvent } from 'src/types';
...
export class NatsService implements OnModuleInit {
  ...
  async onModuleInit() {
    ...
    await Promise.allSettled([
      this.startConsume(async (message: JsMsg, data: TiktokEvent) => {
        console.log(
      ...
      this.startRespond('reports.tiktok.events', async (filters: GetEventsFilter) => {
        this.logger.log('Received event on reports.tiktok.events');
        return await this.eventsService.getEvents(filters);
      }),
      this.startRespond('reports.tiktok.revenue', async (filters: GetRevenueFilter) => {
        this.logger.log('Received event on reports.tiktok.revenue');
  ...
  async startConsume(cb: (message: JsMsg, data: TiktokEvent) => Promise<void>) {
    await this.createConsumer('ttk-collector');

    try {
      const consumer = await this.jetStream.consumers.get('events', 'ttk-collector');

      const messages = await consumer.consume({ max_messages: 1000 });

      for await (const message of messages) {
        try {
          const data = JSON.parse(this.stringCodec.decode(message.data)) as TiktokEvent;
